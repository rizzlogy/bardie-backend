...
class BardAI {
  ...
  async configure(keyType, apiKey) {
    ...
    if (!key) {
      return {
        content: "Invalid session type. Use 1 for __Secure-1PSID or 2 for __Secure-3PSID session type.",
        status: false,
        creator: "RizzyFuzz",
      };
    }
  ...
  async questionWithImage(input_text, imageBuffer) {
    try {
      ...
      if (imageBuffer) {
        const imageLocation = await this.uploadImage(`bard-ai_upload`, imageBuffer);
        input_text_struct[0].push(0, null, [[imageLocation, 1], "bard-ai_upload"]);
      }
  ...
  async uploadImage(name, url) {
    try {
      ...
      const finalizeResponse = await axios.put(uploadUrl, imageResponse.data, {
        headers: {
  ...
  async setSnim0e() {
    try {
      ...
      if (!response.headers["set-cookie"]) {
        return {
          content: "Invalid __Secure-1PSID or __Secure-3PSID value. Please ensure you provide a valid __Secure-1PSID or __Secure-3PSID value.",
          status: false,
          creator: "RizzyFuzz",
        };
      }
      ...
      if (match) {
        ...
        console.log("[ ✖️ ] Failed!\n");
        return {
          content: "Failed to retrieve SNlM0e pattern. Please ensure you provide a valid __Secure-1PSID or __Secure-3PSID value.",
          status: false,
          creator: "RizzyFuzz",
        };
      }
