...
class Bard {
  constructor(timeout = 10000, proxies = null) {
    ...
    this.params = {
      bl: "boq assistant-bard-web-server_20240130.03_p0",
  ...
  async question(message) {
    try {
      ...
      if (!this.data.at) {
        return {
          content:
            "Authentication Error, Contact The Admin: support@rizzy.eu.org",
          status: false,
          creator: "RizzyFuzz",
        };
      }
      ...
      if (chatData === null) {
        return {
          content: `Limited Request, Please Try Again Later`,
          status: false,
          creator: "RizzyFuzz",
        };
      }
  ...
  async setSnim0e() {
    try {
      ...
      if (response.status !== 200) {
        return { content: `Failed to connect in bard server` };
      }

      if (!response.headers["set-cookie"]) {
        return {
          content: "Cookie Not Set, Contact The Admin: support@rizzy.eu.org",
          status: false,
          creator: "RizzyFuzz",
        };
      }
      ...
      if (match) {
        ...
        return {
          content:
            "Failed to retrieve SNlM0e pattern. Contact The Admin: support@rizzy.eu.org",
          status: false,
          creator: "RizzyFuzz",
        };
      }
