...
class BardAI {
  ...
  async questionWithImage(input_text, url) {
    try {
      if (!this.data.at) {
        console.log("[ ✖️ ] Failed!\n");
        return {
      ...
      let imageLocation = await this.uploadImage(`bard-ai_upload.jpg`, url);
      const input_text_struct = [
        [input_text],
        null,
        [[[imageLocation, 1], "bard-ai_upload.jpg"]],
      ];
  ...
  async uploadImage(name, url) {
    try {
      ...
      let { data: buffer } = await this.session.get(url, {
        headers: {
          "User-Agent":
            "Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.114 Safari/537.36",
        },
      ...
      const imageResponse = await this.session.post(uploadUrl, buffer, {
        headers: {
