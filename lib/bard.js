...
class BardAI {
  ...
  async configure(keyType, apiKey) {
    ...
    if (!key) {
      return {
        ...
        status: false, creator: "RizzyFuzz"
      };
    }
  ...
  async question(input_text) {
    ...
    if (response.status !== 200) {
      return { content: `Response Status: ${response.status}`, status: false, creator: "RizzyFuzz" };
    }

    const resp_dict = JSON.parse(response.data.split("\n")[3])[0][2];

    if (resp_dict === null) {
      return { content: `Response Error: ${response.data}.`, status: false, creator: "RizzyFuzz"  };
    }
  ...
  async setSnim0e() {
    try {
      ...
      if (!response.headers["set-cookie"]) {
        return { content: "Invalid __Secure-1PSID or __Secure-3PSID value. Please ensure you provide a valid __Secure-1PSID or __Secure-3PSID value.", status: false, creator: "RizzyFuzz" };
      }
      ...
      if (match) {
        ...
        return { content: "Failed to retrieve SNlM0e pattern. Please ensure you provide a valid __Secure-1PSID or __Secure-3PSID value.", status: false, creator: "RizzyFuzz" };
      }
    } catch (error) {
      console.error(error);
    }
