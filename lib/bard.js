...
class BardAI {
  ...
  async question(message) {
    try {
      ...
      const parseTextHtml = (text, images) => {
        if (!images) return text;
        for (const imageData of images) {
          const formattedTag = `<img src="${imageData.url}" alt="${imageData.tag
            .replace(/\[.*?\]/g, "")
            .trim()}" style="width: 90%; border: 1px solid black;">`;
          text = text.replace(
      ...
      return {
        result: {
          content: parseText(text, images),
          contentWithHtml: parseTextHtml(text, images),
        },
        status: true,
        creator: "RizzyFuzz",
      };
  ...
  async questionWithImage(message, url) {
    try {
      ...
      const parseTextHtml = (text, images) => {
        if (!images) return text;
        for (const imageData of images) {
          const formattedTag = `<img src="${imageData.url}" alt="${imageData.tag}" style="width: 90%">`;
          text = text.replace(
            new RegExp(`(?!\\!)\\[${imageData.tag.slice(1, -1)}\\]`),
            formattedTag,
          );
