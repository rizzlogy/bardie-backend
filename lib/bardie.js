class Bard {
    ...
    async #init(cookie) {
        ...
        try {
            ...
            throw new Error("Could not use your Cookie. Make sure that you copied the Cookie with the name __Secure-1PSID correctly. If you are sure your cookie is correct, you may also have reached your rate limit.");
        }
    }
    ...
    async #uploadImage(name, buffer) {
        ...
        try {
            ...
            throw new Error("Could not upload the image to Google Bard. Make sure you are connected to the internet: " + e);
        }
    }
    ...
    async #query(message, config) {
        ...
        // If IDs are provided but don't have every one of the expected IDs, error
        const messageStruct = [
            [message],
            null,
            [null, null, null],
        ...
        // Get the first Bard-recommended answer
    ...
    async #parseConfig(config) {
        ...
        // Verify that the format is one of the two types
        if (config?.format) {
            switch (config.format) {
                ...
                case Bard.MD:
                    ...
                    break;
                default:
                    throw new Error("Format can only be Bard.JSON for JSON output or Bard.MD for Markdown output.");
            }
        }
